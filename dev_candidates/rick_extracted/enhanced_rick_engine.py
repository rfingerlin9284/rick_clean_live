...
class EnhancedStochasticEngine:
    ...
    async def execute_enhanced_trade(self):
        ...
        if is_winner:
            ...
            pnl = reward + (extra_profit * position_size if 'extra_profit' in locals() else 0)
            outcome = 'win'
            self.wins += 1
        else:
            # Loss - hit stop loss
    ...
    async def generate_enhanced_report(self):
        ...
        charter_compliance = {
            'min_notional_met': all(t.notional_usd >= self.min_notional for t in self.trades),
            'rr_compliance': all(abs(t.take_profit - t.entry_price) / abs(t.stop_loss - t.entry_price) >= RickCharter.MIN_RISK_REWARD_RATIO for t in self.trades),
            ...
            'oco_latency_compliance': all(t.oco_latency_ms <= RickCharter.MAX_PLACEMENT_LATENCY_MS for t in self.trades)
        }
...
async def main():
    ...
    print("🏆 Enhanced RICK Stochastic Engine")
    print("Full Charter Compliance: $15K notional, dynamic leverage, ATR stops, OCO, trailing")
    print("=" * 80)

    try:
        engine = EnhancedStochasticEngine(pin=PIN)
        await engine.run_enhanced_test(duration_minutes=10)  # 10-minute comprehensive test

        print("\n📁 Enhanced test report saved to: logs/enhanced_rick_report.json")
        print("📊 Full charter compliance validation complete")
