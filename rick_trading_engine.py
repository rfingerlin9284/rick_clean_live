...
class PaperTradingEngine(CharterCompliantPaperEngine):
    ...
    async def execute_charter_compliant_trade(self, signal: Dict):
        ...
        trade_id = f"PAPER_CHARTER_{len(self.trades)}_{int(__import__('time').time())}"
            ...
            if hedge:
                ...
                log_narration(
                    ...
                    details={
                        ...
                        "note": "Auto-hedging disabled in PAPER mode (testing only)"
                    },
                    symbol=signal['symbol'],
                    venue="oanda_practice"
                )
                
                # NOTE: Auto-hedge placement is DISABLED in PAPER mode for testing
    ...
    async def generate_final_report(self):
        ...
        report = {
            ...
            'promotion_eligible': (
                completed_trades >= 3 and  # PAPER requires minimum 3 trades (45 min session)
...
async def main():
    ...
    await engine.start_paper_trading()  # Uses same trading loop
